...
function Book(title, author, pages) {
    ...
    this.read = function(span) {
        this.status = "Read";
        span.textContent = `Status : ${this.status}`;
    }
}
...
function display() {
    ...
    myLibrary.forEach((book) => {
        ...
        const id = createSpan("ID", book.id);
        const title = createSpan("Title", book.title);
        const author = createSpan("Author", book.author);
        const pages = createSpan("Pages", book.pages);
        const status = createSpan("Status", book.status);
